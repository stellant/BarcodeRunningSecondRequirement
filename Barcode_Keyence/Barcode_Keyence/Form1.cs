    ...
    public partial class Form1 : Form
    {
        ...
        private void updateDataValue(byte[] data)
        {
            try
            {
                ...
                string dateString = DateTime.Now.ToString("yyyyMMddHHmmssfff")+"Tz"+TimeZone.CurrentTimeZone.GetUtcOffset(DateTime.Now);
                ...
                if (!dataString.StartsWith("ERROR"))
                {
                    ...
                    destinationString = Path.Combine(folderPath,dataString+"_"+dateString+"."+Path.GetExtension(imageString.Split('\\')[2]));
        ...
        private void button_FileChoose_Click(object sender, EventArgs e)
        {
            ...
            if (saveFileDialog1.ShowDialog().Equals(DialogResult.OK))
            {
                ...
                fileNameNew = fileName + "_" + DateTime.Now.ToString("yyyyMMddHHmmssfff");
                textBox_FileName.Text = Path.GetFileNameWithoutExtension(filePath);
                WriteLog("Data will be written to " + Path.GetDirectoryName(filePath) + "   at" + DateTime.Now.ToString("yyyyMMddHHmmssfff") + "\n");
